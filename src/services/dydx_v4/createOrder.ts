...
export const dydxV4CreateOrder = async (orderParams: dydxV4OrderParams) => {
	...
	const doLimitOrder = async (
		...
		let totalFilled = 0;
		let remainingOrderSize = size - totalFilled;
		while (count > 0) {
	...
	if (orderParams.limitOrderAttempts && orderParams.limitOrderSeconds) {
		...
		if (orderParams.trdmAlert === "LIMIT THEN MARKET") {
			...
			const attemptLimitOrder = await doLimitOrder(limitSeconds,orderParams.limitOrderAttempts);
			...
			if (remaining > 0) {
				...
				const attemptMarketOrder = await doMarketOrder(remaining);
	...
	if (orderParams.trdmAlert === "MARKET CLOSE") {
		...
		const attemptMarketOrder = await doMarketOrder(size, true);
		res.push(attemptMarketOrder);
	}
